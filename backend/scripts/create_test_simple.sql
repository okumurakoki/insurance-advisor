...
FROM users WHERE user_id = 'test-agency' AND account_type = 'parent'
ON CONFLICT (user_id, account_type) DO NOTHING;
...
WHERE u.user_id = 'test-staff' AND u.account_type = 'child';

-- 確認
SELECT user_id, name, account_type FROM users WHERE user_id IN ('test-agency', 'test-staff');
